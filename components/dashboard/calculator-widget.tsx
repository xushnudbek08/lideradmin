...
interface CalculatorWidgetProps {
  onCalculate?: (amount: number, term: number, rate: number, commission: number) => void
  role?: "client" | "agent"
}
...
export function CalculatorWidget({ onCalculate, role = "client" }: CalculatorWidgetProps) {
  ...
  const rate = getRate()
  // Расчет комиссии в зависимости от срока (формула будет обновлена позже)
  ...
  // Автоматически применяем значения при изменении
  const handleAmountChange = (value: number) => {
    setGuaranteeAmount(value)
    if (onCalculate) {
      const newRate = getRate()
      const newCommission = (value * newRate * term) / 100 / 12
      onCalculate(value, term, newRate, newCommission)
    }
  }
  ...
  const handleTermChange = (value: number) => {
    ...
    if (onCalculate) {
      const newRate = getRate()
      const newCommission = (guaranteeAmount * newRate * value) / 100 / 12
      onCalculate(guaranteeAmount, value, newRate, newCommission)
    }
  }
  ...
  const handlePurchaseTypeChange = (value: string) => {
    ...
    if (onCalculate) {
      const newRate = getRate()
      const newCommission = (guaranteeAmount * newRate * term) / 100 / 12
      onCalculate(guaranteeAmount, term, newRate, newCommission)
    }
  }

  const handleGuaranteeTypeChange = (value: string) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="text-3xl font-bold text-foreground">{formatNumber(guaranteeAmount)}</div>
                <Slider
              ...
              {/* Purchase Type */}
              <div className="space-y-3">
                <RadioGroup value={purchaseType} onValueChange={handlePurchaseTypeChange} className="flex gap-2">
                  <div key="44-fz" className="flex-1">
                    <RadioGroupItem value="44-fz" id="calc-44-fz" className="peer sr-only" />
                    <Label
                      htmlFor="calc-44-fz"
                      className="flex items-center justify-center px-3 py-2.5 border-2 border-border rounded-md cursor-pointer text-sm font-medium text-foreground peer-data-[state=checked]:border-destructive peer-data-[state=checked]:text-destructive peer-data-[state=checked]:bg-destructive/10 hover:bg-secondary transition-all"
                    >
                      44-ФЗ
                    </Label>
                  </div>
              ...
              {/* Guarantee Type */}
              <div className="space-y-3">
                <RadioGroup value={guaranteeType} onValueChange={handleGuaranteeTypeChange} className="flex gap-2">
                  <div key="execution" className="flex-1">
                    ...
                    <Label
                      ...
                      className="flex items-center justify-center px-2 py-2.5 border-2 border-border rounded-md cursor-pointer text-sm font-medium text-foreground peer-data-[state=checked]:border-destructive peer-data-[state=checked]:text-destructive peer-data-[state=checked]:bg-destructive/10 hover:bg-secondary transition-all"
                    >
                      ИСПОЛНЕНИЕ
                    </Label>
                  </div>
              ...
              <div className="pt-2">
                ...
                <div className="text-2xl font-bold text-destructive mt-2">{formatCurrency(commission)}</div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="text-3xl font-bold text-foreground">{term}</div>
                <Slider
                  value={[term]}
              ...
              <div className="space-y-3">
                ...
                <div className="text-3xl font-bold text-foreground">{rate.toFixed(2)}</div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="text-lg text-foreground mt-2">—</div>
              </div>
            </div>
          </div>
      ...
      <CardContent className="p-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Slider value={[term]} onValueChange={([value]) => handleTermChange(value)} min={6} max={84} step={1} />
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>6 мес.</span>
              <span>84 мес.</span>
            </div>
